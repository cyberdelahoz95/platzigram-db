...
class Db {
  ...
  getImage (id, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      if (!image) {
        return Promise.reject(new Error(`ìmage ${imageId} not found`))
      }
  ...
  getImagesByUser (userId, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      yield r.db(db).table('images').indexWait().run(conn) // al declarar indeces se utiliza esta function para esperar a que los indices qye se declararn arriba realmente existan

      let images = yield r.db(db).table('images').getAll(userId, { index: 'userId' }).orderBy(r.desc('createdAt'))
       .run(conn) // esto devuelve un cursor, el cual es iterado o puede ser convertido a array o json mas adelante
